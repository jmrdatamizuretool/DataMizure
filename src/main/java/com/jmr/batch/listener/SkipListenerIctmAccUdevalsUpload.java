...
@Component
public class SkipListenerIctmAccUdevalsUpload {
	@Autowired
	...
	@OnSkipInProcess
	public void skipInProcess(TransIctmAccUdevalsUploadModel transIctmAccUdevalsUploadModel, Throwable th) {
		String path = getErrorLog();
		String process_err_path = path + System.getProperty("file.separator") + "SkipInProcess.txt";
		createFile(process_err_path, 
				transIctmAccUdevalsUploadModel.getAcc());
	}
	
	
	@OnSkipInWrite
	public void skipInWriter(TransIctmAccUdevalsUpload transIctmAccUdevalsUpload, Throwable th) {
		String path = getErrorLog();
		String write_err_path = path + System.getProperty("file.separator") + "SkipInWrite.txt";
		createFile(write_err_path, 
				transIctmAccUdevalsUpload.getAcc());
	}
	...
	public String getErrorLog() {
		Optional<ErrorLogVb> errorLogVb = repo.findById("ICTM_ACC_UDEVALS_UPLOAD");
