...
@Component
public class SkipListenerBctbUploadShipmentInfo {
	...
	@OnSkipInProcess
    public void skipInProcess(TransBctbUploadShipmentInfoModel model, Throwable th) {
        String path = getErrorLog();
    ...
    @OnSkipInWrite
    public void skipInWriter(TransBctbUploadShipmentInfo entity, Throwable th) {
        String path = getErrorLog();
        String writeErrPath = path + System.getProperty("file.separator") + "SkipInWrite.txt";

        String data = "SOURCE_CODE:\t" + entity.getTransBctbUploadShipmentInfoPK().getSourceCode() + "\tSOURCE_CODE:" +
                      "BRANCH_CODE:\t" + entity.getTransBctbUploadShipmentInfoPK().getBranchCode() + "\tBRANCH_CODE:" +
                      "SOURCE_REF:\t" + entity.getTransBctbUploadShipmentInfoPK().getSourceRef() + "\tSOURCE_REF:" +
                      "SOURCE_SEQ_NO:\t" + entity.getTransBctbUploadShipmentInfoPK().getSourceSeqNo() + "\tSOURCE_SEQ_NO:" +
                      "\tException:" + th.getMessage();
	...
	public String getErrorLog() {
		Optional<ErrorLogVb> errorLogVb = repo.findById("BCTB_UPLOAD_SHIPMENT_INFO");
